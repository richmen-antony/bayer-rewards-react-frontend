...
type ICountryProps = {
  ...
  devconfig: any;
  restrictCountryAPI: boolean;
  handleRestrictCountryAPI: () => void;
};
...
const CountrySetupComp = (props: ICountryProps) => {
  ...
  }, [countryCode, restrictCountryAPI]);
  ...
  const getTemplateByCountry = (countryShortName: any) => {
    ...
    invokeGetAuthService(getTemplateData, data)
      .then((response: any) => {
        ...
        props.addPackagingDefinitionInputList(objCountryData.productpackagedefinition);
        props.addScanpointsAndAllocationInputList(objCountryData.scanpointallocationdefinition);

        props.setAnticounterfeitSmsAuthentication(objCountryData.smsauthentication);
    ...
    if (newCountry === true) {
      props.addLocationInputList([{ level: 0, name: "", parentlevel: -1 }]);
      props.addRoleInputList([
        {
          level: 0,
          code: "",
  ...
  const _retriveCountryCode = (countryValue: any) => {
    ...
    // When clicked the back button and handle updated data with input list
    if (!restrictCountryAPI) {
      ...
      <div className="container">
        <div className="row effectiveDate fo  rm-group">
          <div className="col-sm-3">
            <div>
              <label className="font-weight-bold pt-4 label">
                <FormattedMessage id="devAdmin.country" />
              </label>
            </div>
            <div>
              {/* <Select
          ...
          <div className="col-sm-3">
            <div>
              <label className="font-weight-bold pt-4 label">
                <FormattedMessage id="devAdmin.countryCode" />
              </label>
            </div>
            <div>
              <input className="form-control dpstyle" type="text" name="Currency" value={countryISO} read-only />
            </div>
          </div>
          ...
          <div className="col-sm-3">
            <div>
              <label className="font-weight-bold pt-4 label">
                <FormattedMessage id="devAdmin.currencyCode" />
              </label>
            </div>
            <div>
              <input
          ...
          <div className="col-sm-3">
            <div>
              <label className="font-weight-bold pt-4 label">
                <FormattedMessage id="devAdmin.currency" />
              </label>
            </div>
            <div>
              <input type="text" className="form-control dpstyle" name="Currency" value={currencyDesc} />
            </div>
          </div>
        </div>
      </div>
...
export const CountrySetup = connect(mapStateToProps, mapDispatchToProps)(CountrySetupComp);
