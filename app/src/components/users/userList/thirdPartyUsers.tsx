...
class ChannelPartners extends Component<Props, States> {
  ...
  handleSort(
    ...
    this.props.onSort(columnname, allThirdPartyUsers, isAsc);
  }
  ...
  generateHeader(allThirdPartyUsers: any, isAsc: Boolean) {
    ...
    res.push(
      <th
        style={{ width: "9%" }}
        onClick={(e) =>
          this.handleSort(e, "username", allThirdPartyUsers, isAsc)
        }
        key="username"
      >
        {"USER NAME"}
    ...
    res.push(
      <th
        style={{ width: "10%" }}
        onClick={(e) =>
          this.handleSort(e, "ownerphonenumber", allThirdPartyUsers, isAsc)
        }
    ...
    res.push(
      <th
        style={{ width: "12%" }}
        onClick={(e) =>
          this.handleSort(e, "whtaccountname", allThirdPartyUsers, isAsc)
        }
        key="whtaccountname"
      >
        {"FULL NAME"}
    ...
    res.push(
      <th
        style={{ width: "12%" }}
        onClick={(e) =>
          this.handleSort(e, "ownerfirstname", allThirdPartyUsers, isAsc)
        }
        key="ownerfirstname"
      >
        {"PARTNERS MAPPED"}
  ...
  handleChange = (idx: any, e: any, key: string, type: string, val: any) => {
    const { allThirdPartyUsers } = this.props;
...
asahandleRemoveSpecificRow = (idx: any) => () => {
  ...
  render() {
    const { isAsc, totalData, geolevel1List } = this.props;
    const { isLoader, pageNo, rowsPerPage } = this.props.state;
    const { userList, userData, isStaff,allThirdPartyUsers, partnerDatas }: any = this.state;
