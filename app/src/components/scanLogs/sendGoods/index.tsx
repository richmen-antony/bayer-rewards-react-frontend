  ...
  handleReactSelect = (selectedOption: any, e: any, inActiveFilter?: boolean) => {
    ...
    if (inActiveFilter) {
      ...
      if (selectedOption.value !== this.state[e.name]) {
        ...
        if (stateFilterName === "selectedDistributorFilters") filters["soldtoid"] = "ALL";
        //If scantype update after set Distributor or Retailer dropdown value is all(Default vaue)
        if (stateFilterName === "selectedWarehouseFilters") filters["customerid"] = "ALL";
        oneTimeAPI = true;
      }
      let scanTypeListValue: any[] = [...scanTypeList];
      if (e.name === "selectedScannedBy") {
        scanTypeListValue =
          selectedOption.value === "Distributor" ? [...distributorScanTypeList] : [...warehouseScanTypeList];
                    ...
                    <Fragment>
                      <div className="form-group" onClick={(e) => e.stopPropagation()}>
                        <ReactSelect
                          ...
                          label={`Customer Name (${
                            this.state.selectedScanType === "SG - D2R" ? "Retailer" : "Distributor"
                          })`}
                      ...
                      <div className="pt-1">
                        {this.state.status.map((item: any, statusIndex: number) => (
                          <span className="mr-2" key={statusIndex}>
                            <Button
                              color={
                                filter.scanstatus === item ? "btn activeColor rounded-pill" : "btn rounded-pill boxColor"
                              }
                              size="sm"
                      ...
                      <div className="pt-1">
                        {this.state.goodStatus.map((item: any, statusIndex: number) => (
                          <span className="mr-2" key={statusIndex}>
                            <Button
                              color={
                                filter.dispatchstatus === item.value
                                  ? "btn activeColor rounded-pill"
                                  : "btn rounded-pill boxColor"
                              }
                              size="sm"
